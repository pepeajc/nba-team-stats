...
export function getSlugName(type: 'game' | 'player' | 'team', name: string): string {
...
export function getShortName(type: 'game' | 'player' | 'team', slug: string): string {
...
export function formatESPNGames(events: any[]): Game[] {
  return events
    .map((event) => {
      const competition = event.competitions?.[0] || event.boxscore?.teams
...
export function formatESPNScores(events: any[]): ScoreboardProps[] {
  return events
    .map((event) => {
      ...
      const homeTeam = competition[event.boxscore?.teams.find((c: any) => c.homeAway === "home").displayOrder - 1]
      const awayTeam = competition[event.boxscore?.teams.find((c: any) => c.homeAway === "away").displayOrder - 1]

      if (!homeTeam || !awayTeam) return null

      return {
        gameId: getJSONField(event.header, 'id'),
        gameTimeUTC: getJSONField(event.meta, 'firstPlayWallClock') || new Date().toISOString(),
        shortName: homeTeam.team?.displayName + " vs " + awayTeam.team?.displayName,
        homeTeam: {
          teamId: homeTeam.team.id,
          teamName: homeTeam.team?.displayName || "",
          teamTricode: homeTeam.team?.abbreviation || "",
          score: Number.parseInt(homeTeam.statistics[0].totals[1] || "0"),
        },
            awayTeam: {
            teamId: getJSONField(awayTeam.team, 'id'),
            teamName: awayTeam.team?.displayName || "",
            teamTricode: awayTeam.team?.abbreviation || "",
            score: Number.parseInt(awayTeam.statistics[0].totals[1] || "0"),
            },
      }
    })
